...
class EmployeeStartTimeViolation(Violation):
    """
    A task performed before the assigned employee's working-time lower bound.
    """

    def __init__(self, task: Task, employee: Employee, start_time: int, end_time: int):
        """
        Args:
            task: Task performed before the employee's working-time lower bound.
    ...
    @property
    def task(self):
        """Task performed before the employee's working-time lower bound."""
        return self._task
