...
| `TaskDeletion(freed_employees, candidate_tasks, min_nb_removals=1, max_nb_removals=1)` | Removes between `min_nb_removals` and `max_nb_removals` candidate tasks from a candidate employee's sequence, freeing `freed_employees`' other tasks to shift in time/get reordered. | $\mathcal{E} :=$ freed_employees, $\mathcal{J} :=$ candidate_tasks, $n^- :=$ min_nb_removals, $n^+ :=$ max_nb_removals | $\mathcal{E} \cup \mathcal{J}$ | $n^- \le \|\mathcal{J}\| - \sum_{j \in \mathcal{J}} X_j \le n^+$, $\sum_{k} U_{e^0(j),\,j,\,k} = X_j \ \ \forall\, j \in \mathcal{J}$                  | Never gets slack variables of its own. `NeighborhoodFeasibilityMILP` only accepts `TaskDeletion` paired with a gap-bearing operator (`TaskInsertion`/`TaskRepositioning`/`SequenceReordering`).                       |
...
the neighborhood pipeline's feasibility gap is only guaranteed to be at or below the tailored pipeline's, 
...
so it can only find an equal or smaller gap. 
Both pipelines agree exactly whenever either finds a fully feasible (gap 0) arrangement.