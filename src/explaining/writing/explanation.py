...
def define_single_explanation_json_file_name(question: Question):
    return f"explanation_{question.solution.short_name}_{question.template.id}{question.fields_values}.json"


def export_single_explanation_to_json_file(explanation: Explanation, output_directory: str = None):
    file_name = define_single_explanation_json_file_name(explanation.question)
...
def define_multiple_explanations_json_file_name(explanations: list[Explanation]):
    explanation = explanations[0]
    return f"explanations_{explanation.question.solution.short_name}.json"


def export_multiple_explanations_to_json_file(explanations: list[Explanation], output_directory: str = None):
    file_name = define_multiple_explanations_json_file_name(explanations)
    if output_directory is None:
        output_directory = OUTPUTS_DIRECTORY_RELATIVE_PATH
    file_path = f"{output_directory}/{file_name}"
    explanations_dicts = []
