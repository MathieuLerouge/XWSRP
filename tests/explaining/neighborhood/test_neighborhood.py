...
def test_neighborhood_with_operator_on_order_fixed_employee_is_accepted():
    ...
    assert neighborhood.scope == frozenset({employee, task})
    assert neighborhood.operators == [operator]


def test_neighborhood_scope_aggregates_across_operators():
    instance = build_instance()
    solution = Solution(instance)
    ...
    insertion = TaskInsertion(frozenset({employee_1}), frozenset({task_1}))
    deletion = TaskDeletion(frozenset({employee_2}), frozenset({task_2}))
    neighborhood = Neighborhood(solution=solution, operators=[insertion, deletion])
    assert neighborhood.scope == frozenset({employee_1, employee_2, task_1, task_2})
